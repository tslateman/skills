...
Write the generated skill into the skills directory this repo already uses. Look for `.claude/skills/`, `.cursor/skills/`, or `.agents/skills/`, and use the one that exists. When none exists, create the one your agent reads. `<skills-dir>` below means whichever you picked.
...
`testing`, `test-first`, and `legacy` prove code against a suite. This skill
proves the app against a user. Reach for it when a green suite still leaves you
unsure the feature works.

The generated skill is what `demo` drives when it records an mp4, and what
`maintain-verification-skill` audits as the app changes.
