...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map, under `.claude/skills/`, `.cursor/skills/`, or `.agents/skills/`. Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target.
...
Upkeep for whatever `create-verification-skill` generated. Run it when the app
has moved and the feature map has not.
