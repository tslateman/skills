...
Companion to `zoom-out` and `system-map`, which tell you what the code does and where it sits. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR body and the commits behind it, not just the working tree.
...
6. For a big or wide change, run it as an `arena`. Ask several agents the same question from different angles and merge the answers. Independent readings catch different real bugs.
...
Run it through `prose` before it goes anywhere public, cite real code, and strip anything private.

**Reply:** the writeup above, with the one safety fact either proven or marked unproven.

## Where this sits

`/code-review` hunts defects inside the diff. `vibe-check` asks whether the
change holds up. Blast radius asks what it breaks outside the diff, and refuses
to answer from reading alone.
