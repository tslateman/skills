...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. Add new offenders to `prose`'s LLM-ism catalog with their replacement.
...
- Run `slop-check` over any doc this skill touches, then `prose` to fix what it names. Those two own the slop-pattern catalog: AI vocabulary, filler, hedging, formatting tells.
...
This skill decides what kind of document you are writing. The rest of the group
decides whether the sentences are ready.

| Reach for           | When                                                    |
| ------------------- | ------------------------------------------------------- |
| `technical-writing` | Choosing the mode, or reviewing a doc set for structure |
| `prose`             | Tightening a draft you have already framed              |
| `slop-check`        | Scoring a finished draft before it goes out             |
| `ste`               | A procedure the reader executes, under full ASD-STE100  |

`ste` is the deeper authority on the instruction rules this skill summarises.
When the document is a runbook, migration, or incident procedure, go there
instead of working from the STE section here.
